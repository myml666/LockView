...
{
	private boolean mCheckAnwser=false;
	 ...
	 */
	private int mFingerUpColorFiled = 0xFFFF0000;//失败
	private int mFingerUpColorSuccess = 0xFF00FF00;//成功
	/**
	 * GestureLockView填充圆的颜色
	 */
	private int mFingerColorFill = 0xFFFF0000;
	/**
	 * 宽度
	 */
	private int mWidth;
			...
			int defStyle)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			case R.styleable.GestureLockViewGroup_color_finger_on:
				...
				break;
				case R.styleable.GestureLockViewGroup_color_finger_fill:
					mFingerColorFill = a.getColor(attr, mFingerColorFill);
					break;
			case R.styleable.GestureLockViewGroup_color_finger_up_filed:
				mFingerUpColorFiled = a.getColor(attr, mFingerUpColorFiled);
				break;
				case R.styleable.GestureLockViewGroup_color_finger_up_success:
					mFingerUpColorSuccess = a.getColor(attr, mFingerUpColorSuccess);
					break;
		...
		if (mGestureLockViews == null)
		{
			...
			for (int i = 0; i < mGestureLockViews.length; i++)
			{
				...
				mGestureLockViews[i] = new GestureLockView(getContext(),
						...
						mFingerOnColor, mFingerUpColorSuccess,mFingerUpColorFiled,mFingerColorFill);
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_UP:
			mCheckAnwser=checkAnswer();
			if(mCheckAnwser){
				mPaint.setColor(mFingerUpColorSuccess);
			}else {
				mPaint.setColor(mFingerUpColorFiled);
			}
			mPaint.setAlpha(50);
			this.mTryTimes--;
			...
			{
				mOnGestureLockViewListener.onGestureEvent(mCheckAnwser);
				if (this.mTryTimes == 0)
				{
	...
	private void changeItemMode()
	{
		for (GestureLockView gestureLockView : mGestureLockViews)
		{
			...
			{
				if(mCheckAnwser){
					gestureLockView.setMode(Mode.STATUS_FINGER_UP_SUCCESS);
				}else {
					gestureLockView.setMode(Mode.STATUS_FINGER_UP_FILED);
				}
			}
		}
	}
